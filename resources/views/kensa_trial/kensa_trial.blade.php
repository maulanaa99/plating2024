...
@section('breadcrumb')
    @parent
    <li class="active"> -> Trial > Kensa Trial</li>
@endsection
    ...
    <div class="card-header" style="padding-left: 10px;">
        <div class="row float-right">
            <div class="col-12 col-md-12 col-lg-12">
                <a href="{{ route('tr.kensa.tambah') }}" class="btn btn-icon icon-left btn-info"><i class="fas fa-plus"></i>
                    Tambah Data</a>
            </div>
        </div>
        <form action="{{ route('tr.kensa') }}" method="GET">
            <div class="row">
                <div class="col-md-4">
        ...
        <table id="kensa-table" class="table table-sm table-hover table-bordered table-striped responsive">
            <thead>
                <tr>
                    ...
                    <th rowspan="2" class="align-middle text-center">Jam Unracking</th>
                    <th rowspan="2" class="align-middle text-center">Tgl Kensa</th>
                    <th rowspan="2" class="align-middle text-center">Jam Kensa</th>
            ...
            <tbody>
                @foreach ($kensa_trial as $no => $kensha)
                    <tr>
                        <td>{{ $no + 1 }}</td>
                        ...
                        <td>
                            ...
                            @elseif($kensha->status == '7')
                                <span class="btn btn-success">Approved</span>
                            @elseif($kensha->status == '8')
                                <span class="btn btn-danger">Cancell</span>
                            @endif
                        </td>
                        </td>
                        @if (Auth::user()->role == '1')
                            <td>
                                @if ($kensha->status == '6')
                                    <a href="{{ route('tr.kensa.approve', $kensha->id) }}" class="btn btn-sm btn-success">
                                        <i class="fa fa-check"></i> </a>
                                    <a href="{{ route('tr.kensa.cancell', $kensha->id) }}" class="btn btn-sm btn-danger">
                        ...
                        success: function(data) {
                            if (data.success) {
                                Swal.fire(
                                    ...
                                    window.location.href = '/kensa_trial';
                                });
                            } else {
                                Swal.fire(
                                    'Gagal!',
